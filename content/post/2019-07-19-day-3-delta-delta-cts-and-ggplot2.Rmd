...
Now that we have our pipe-line template set up, we can start adding some dplyr verbs to pull our specifically the control samples. We know the control samples all have "-" in their `Sample Name`, so we can use `filter()` and `str_detect()` to isolate these samples.
...
Open the table using the RStudio viewer to check that now the only samples in the table are control samples.
...
I've decided to make a new data frame here called deltadeltaCT. You could just add the $\Delta\Delta$Ct calculation onto the pipe that we made last week, but I felt that this was a logical place to make a new data frame; sometimes longer pipe-lines can become difficult to interpret.
...
## Plotting with `ggplot2`[ADD LINK HERE]
...
To make a `ggplot`, we can use the `ggplot()` function. The first argument is the data frame we want to plot. The next argument is a series of __aesthetic mappings__. These are things like the x and y values of the plot, groups in the data, and other things. 
...
Next, to tell `ggplot()` what kind of plot to make (scatter plot), we add the specification `geom_point()` using the `+` operator. The `+` operator is kind of like the pipe operator (`%>%`) in that you use it to build on to the code that came before it-- but this is not a perfect analogy. The `+` is specific to building plots with `ggplot()`.

`ggplot()` makes different kinds of plots using different __geometries__. So we specified a scatter plot using the point geometry (`geom_point()`). Some other common geometries are `geom_boxplot()`, `geom_bar()`, and `geom_line()`.
...
Now tell `ggplot2` to color the points by our grouping variable by adding `color = SampleType` to the `aes()` in the `ggplot()` we built:
...
Another solid improvement. But it's still hard to see some of the overlapping points. There are actually two good solutions to this issue. 
 ...
 2. We can add some random noise to the points so they spread our by switching our `position` argument to `jitterdodge()`.
