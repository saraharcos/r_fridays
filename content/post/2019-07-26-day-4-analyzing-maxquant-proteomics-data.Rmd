...
The data we analyzed today is comes from an LC-MS/MS analysis of the human RNA-binding proteins that directly interact with incoming RNA virus genomes. You can use [this link](https://saraharcos.github.io/projects.html) to see more about how this data was generated _(manuscript under review)_.
...
One more thing before we go: to see the +IFN treated and untreated samples in separate plots we split the actual dataframe into two separate dataframes. ggplot2 has a useful function called "faceting" that would allow us to keep all of our data in a single dataframe.
...
With distribution plots, it is it is often more useful to compare across the x axis than the y, so we can use the `nrow` argument in `facet_wrap()` to force ggplot to show the +IFN Pretreated and Untreated plots on top of one another, rather than next to one another.
